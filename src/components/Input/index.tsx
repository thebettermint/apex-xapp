...
const Input = forwardRef(
  (
    ...
    const [changeEvent, passChangeEvent] = useState<React.ChangeEvent<HTMLInputElement>>();
    ...
    const handleOnPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
      if (type == 'secretnumber' && e.clipboardData.getData('text')) {
        let number = Number(e.clipboardData.getData('text'));
        if (!Number.isInteger(number)) return;
      }
      if (type == 'secretnumber' && e.clipboardData.getData('text').length > 6) return;
    ...
    useEffect(() => {
      if (changeEvent && onChange) onChange(changeEvent);
    }, [changeEvent]);
